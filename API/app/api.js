...
module.exports = function (app, express) {
    ...
    api.post('/Coordinator', function (req, res) {
        ...
        Coordinator.findOne({ 'Email': _newObj.Email }, '', function (err, Obj) {
            if (err)
                return res.json({ code: '1', data: err });
            else {
                if (Obj)
                    return res.json({ code: '20', data: 'Duplicae data, check coordinator email' });
                else {
                    ...
                    var mail = {
                        ...
                        text: 'Dear ' + _newObj.Name + '<br/><br/>\
                                ...
                                <br/>\
                                <b>Please click the following link to confirm your Email</b> then use your login data To login and set your own password.<br/>\
                                <br/>\
                                <a href="' + AppUrl + 'login/' + _newObj.RetrivalCode + '">' + AppUrl + '</a>  \
                                <br/><br/>\
            ...
            if (err)
                ...
                if (Obj && Obj.Email == req.body.Email) {
                    if (Obj.Password == req.body.Password) {
                        ...
                        return res.json({ code: '20', data: 'inncorrect password' });
                    }
                }
                else {
                    return res.json({ code: '21', data: 'no such user' });
                }
            }
        });
    });
