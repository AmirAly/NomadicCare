ehs.controller("BloodglucoseController", function ($scope, $state, $rootScope, $stateParams, API, $timeout) {
    ...
    $('#bloodGlucoseModal').on('hidden.bs.modal', function () {
        $(this).find('form').trigger('reset');
    })

    // start here
    var monthShortNames = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
    var yAxesLabel = 'Blood Glucose (mmol/l)';
    ...
    $scope.$watch('currentClientInfo', function () {
        if (typeof $scope.currentClientInfo === 'undefined') { }
            ...
            $timeout(function () {
                ...
                $rootScope.activetab = 'bloodglucose';
                // Get data
                var req = {
                    method: 'get',
                    url: '/HealthMeasurments/' + $scope.currentClientInfo._id + '/' + 'BloodGlucose',
                    data: {}
                }
                ////loader
                ...
                API.execute(req).then(function (_res) {
                    ...
                    if (_res.data.code == 100) {
                        ...
                        $rootScope.BloodGlucoseArray = _res.data.data;
    ...
    $scope.submit = function (form) {
        angular.forEach($scope.frmNewBloodGlucose.$error.required, function (field) {
            field.$setDirty();
        });
        ...
        if (form.$valid) {
            ...
            API.execute(req).then(function (_res) {
                ...
                if (_res.data.code == 100) {
                    $scope.dismiss();
                    $scope.frmNewBloodGlucose.$setPristine();
                    $state.reload();
                }
                else {
                    $scope.showMessage = true;
    ...
    $scope.SetAnalytics = function (_secondParameter) {
        $rootScope.firstArray = $rootScope.BloodGlucoseArray;
        $rootScope.firstArrayName = 'Blood Glucose';
