ehs.controller("WeightController", function ($scope, $state, $rootScope, $stateParams, API, $timeout) {
    ...
    $scope.$watch('currentClientInfo', function () {
        if (typeof $scope.currentClientInfo === 'undefined') {}
             ...
             $timeout(function () {
                ...
                var req = {
                    method: 'get',
                    url: '/HealthMeasurments/' + $scope.currentClientInfo._id + '/' + 'Weight',
                    data: {}
                }
                ////loader
                ...
                API.execute(req).then(function (_res) {
                    ...
                    if (_res.data.code == 100) {
                        ...
                        $rootScope.WeightArray = _res.data.data;
                ...
                tooltips: {
                    callbacks: {
                        ...
                        label: function (tooltipItem, data) {
                            ...
                            return [yAxesLabel + " " + [obj.Value], "Height " + [obj.Height]];
                        },
    ...
    $scope.submit = function (form) {
        angular.forEach($scope.frmNewWeight.$error.required, function (field) {
            field.$setDirty();
        });
        ...
        if (form.$valid) {
            ...
            API.execute(req).then(function (_res) {
                ...
                if (_res.data.code == 100) {
                    $scope.dismiss();
                    $scope.frmNewWeight.$setPristine();
                    $state.reload();
                }
                else {
                    $scope.showMessage = true;
    ...
    $scope.SetAnalytics = function (_secondParameter) {
        $rootScope.firstArray = $rootScope.WeightArray;
        $rootScope.firstArrayName = 'Weight';
