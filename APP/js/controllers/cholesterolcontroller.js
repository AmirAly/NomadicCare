ehs.controller("CholesterolController", function ($scope, $state, $rootScope, $stateParams, API, $timeout) {
    ...
    $('#cholesterolModal').on('hidden.bs.modal', function () {
        $(this).find('form').trigger('reset');
    })

    // start here
    var monthShortNames = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
    var yAxesLabel = 'Cholesterol (LDL/ HDL/ total md/dl)';
    ...
    $scope.$watch('currentClientInfo', function () {
        if (typeof $scope.currentClientInfo === 'undefined') { }
            ...
            $timeout(function () {
                ...
                $rootScope.activetab = 'cholesterol';
                // Get data
                var req = {
                    method: 'get',
                    url: '/HealthMeasurments/' + $scope.currentClientInfo._id + '/' + 'Cholesterol',
                    data: {}
                }
                ////loader
                ...
                API.execute(req).then(function (_res) {
                    ...
                    if (_res.data.code == 100) {
                        ...
                        $rootScope.CholesterolArray = _res.data.data;
    ...
    $scope.submit = function (form) {
        angular.forEach($scope.frmNewCholesterol.$error.required, function (field) {
            field.$setDirty();
        });
        ...
        if (form.$valid) {
            ...
            API.execute(req).then(function (_res) {
                ...
                if (_res.data.code == 100) {
                    $scope.dismiss();
                    $scope.frmNewCholesterol.$setPristine();
                    $state.reload();
                }
                else {
                    $scope.showMessage = true;
    ...
    $scope.SetAnalytics = function (_secondParameter) {
        $rootScope.firstArray = $rootScope.CholesterolArray;
        $rootScope.firstArrayName = 'Cholesterol';
