ehs.controller("BloodpressureController", function ($scope, $state, $rootScope, $stateParams, API, $timeout) {
    ...
    $('#bloodPressureModal').on('hidden.bs.modal', function () {
        $(this).find('form').trigger('reset');
    })

    // start here
    var monthShortNames = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
    var yAxesLabel = 'Blood Pressure (mmHg)';
    ...
    $scope.$watch('currentClientInfo', function () {
        if (typeof $scope.currentClientInfo === 'undefined') { }
            ...
            $timeout(function () {
                ...
                $rootScope.activetab = 'bloodpressure';
                // Get data
                var req = {
                    method: 'get',
                    url: '/HealthMeasurments/' + $scope.currentClientInfo._id + '/' + 'BloodPressure',
                    data: {}
                }
                ////loader
                ...
                API.execute(req).then(function (_res) {
                    ...
                    if (_res.data.code == 100) {
                        ...
                        $rootScope.BloodPressureArray = _res.data.data;
    ...
    $scope.drawChart = function () {
        ...
        $timeout(function () {
            var ctx = document.getElementById("canvasBloodPressure").getContext("2d");
    ...
    $scope.submit = function (form) {
        angular.forEach($scope.frmNewBloodPressure.$error.required, function (field) {
            field.$setDirty();
        });
        ...
        if (form.$valid) {
            ...
            API.execute(req).then(function (_res) {
                ...
                if (_res.data.code == 100) {
                    $scope.dismiss();
                    $scope.frmNewBloodPressure.$setPristine();
                    $state.reload();
                }
                else {
                    $scope.showMessage = true;
